...
const ForceDirectedGraph = ({ 
  ...
  useEffect(() => {
    ...
    const renderVisualization = () => {
      ...
      // Calculate proper width based on responsive layout
      let availableWidth;
      if (viewportWidth > 1024) {
        // Desktop: visualization takes 75% of container width (minus gap and legend)
        availableWidth = containerRect.width * 0.75 - 24; // 75% minus gap
      } else {
        // Medium and mobile: full container width
        availableWidth = containerRect.width - 16; // Full width minus padding
      }
      
      const svgWidth = width || Math.max(320, availableWidth);
      
      // Responsive height calculation
      let svgHeight;
      if (height) {
        svgHeight = height;
      } else if (viewportWidth <= 675) {
        // Mobile: taller height
        svgHeight = Math.max(500, Math.min(800, svgWidth * 1.0));
      } else if (viewportWidth <= 1024) {
        // Medium screens: moderate height
        svgHeight = Math.max(450, Math.min(650, svgWidth * 0.8));
      } else {
        // Desktop: standard height
        svgHeight = Math.max(400, Math.min(600, svgWidth * 0.75));
      }
      ...
      // Store current simulation reference
      ...
      node
        ...
        .on('click', function (event, d) {
          // Skip click handling for group nodes (color grouping nodes)
          if (d.type === 'group') {
            return;
          }
  ...
  return (
    <div className="flex flex-col items-stretch w-full">
      ...
      <div className="flex flex-col xl:flex-row gap-6 items-start w-full">
        {/* Main Graph - 75% width on desktop, 100% on smaller screens */}
        <div className="w-full xl:w-3/4 rounded-lg relative min-w-0 shadow-lg" ref={containerRef}>
          <svg
            ref={svgRef}
            className="border rounded w-full h-auto"
            style={{ maxWidth: '100%', height: 'auto' }}
          ></svg>
          <div className="absolute top-2 left-2 rounded px-2 py-1 text-xs text-gray-600">
            💡 Drag to pan • Scroll to zoom • Click nodes for details
          </div>
        </div>

        {/* Color Legend - 25% width on desktop, 100% on smaller screens */}
        {legendData.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-4 border w-full xl:w-1/4 xl:flex-shrink-0">
